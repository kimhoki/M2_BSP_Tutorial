...
void DMA0_SW_Init(void)
{
	 ...
	 * (memory -> AHB bus, INC mode, CHK_INT = 0)
	 */
	/* TODO : */ 
	rDISRCC0 &= ~(0x1<<1) & ~(0x1<<0);
	rDIDSTC0 &= ~(0x1<<2) & ~(0x1<<1) & ~(0x1<<0);
	 ...
	 */
	rDCON0 |= (0x1<<31) | (0x1<<30) | (0x1<<29) | (0x1<<27) | (0x1<<24) | (size*4);
...
void DMA0_Timer_Init(void)
{
    ...
    rDCON0 |= (0x1<<31) | (0x1<<30) | (0x1<<29) | (0x1<<27) | (0x1<<23)| (0x3<<24) | (size*4);

	/*****
	rDCON0 = (0<<31)+(1<<30)+(1<<29)+(0<<28)+(1<<27)+(3<<24)+(1<<23)+(0<<22)+(0<<20)+(size*4);
	*****/
...
void DMA0_UART_Init(void)
{
	 ...
	 */
    rDCON0 |= (0x0<<31) | (0x1<<30) | (0x1<<29) | (0x1<<27) | (0x1<<23)| (0x1<<24) | (size*4);
